...
export default function ModernPlusLayout({ data }: { data: ResumeData }) {
    ...
    return (
        <div style={{
            ...
            {/* Header accent stripe — gradient from ac to sc */}
            <div style={{ background: `linear-gradient(90deg, ${ac} 0%, ${sc} 100%)`, height: '5px', width: '100%' }} />

            {/* Header — gradient background */}
            <div style={{
                padding: '2rem 2.5rem 1.5rem',
                background: `linear-gradient(120deg, ${acAlpha(ac, 0.3)} 0%, ${acAlpha(ac, 0.1)} 100%)`,
                borderBottom: `1px solid ${acAlpha(ac, 0.2)}`,
            }}>
                <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start' }}>
                        ...
                        <div style={{ flexShrink: 0 }}>
                            <img
                                ...
                                style={{
                                    ...
                                    border: `3px solid ${ac}`,
                                    display: 'block',
                                    boxShadow: `0 0 0 4px ${acAlpha(sc, 0.25)}`,
                                }}
                            />
                        </div>
                    )}
                    <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {data.title && (
                            <div style={{ fontSize: '0.85rem', color: sc, fontWeight: 600, letterSpacing: '0.05em', marginBottom: '0.8rem' }}>
                                {data.title}
                            </div>
                        )}
                        {hasContacts && (
            ...
            <div style={{ display: 'flex', gap: 0, height: '100%', minHeight: '300mm' }}>
                {/* Left column — gradient tinted */}
                <div style={{
                    width: '35%',
                    padding: '1.5rem',
                    borderRight: `1px solid ${acAlpha(ac, 0.15)}`,
                    background: `linear-gradient(180deg, ${acAlpha(ac, 0.07)} 0%, ${acAlpha(sc, 0.1)} 100%)`,
                }}>
                    {data.summary && (
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.3rem' }}>
                                            {skills.map((sk, si) => (
                                                <span key={si} style={{
                                                    ...
                                                    borderRadius: '3px',
                                                    background: acAlpha(sc, 0.12),
                                                    color: sc,
                                                    fontWeight: 600,
                                                    border: `1px solid ${acAlpha(sc, 0.25)}`,
                                                }}>
                                                    {sk}
                                                </span>
                                            ))}
                            ...
                            {data.education.filter(e => e.school || e.degree).map(edu => (
                                <div key={edu.id} style={{ marginBottom: '0.75rem' }}>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.15rem' }}>
                                        {edu.year && <span style={{ fontSize: '0.62rem', color: sc }}>{edu.year}</span>}
                            ...
                            {data.certifications.map(cert => (
                                <div key={cert.id} style={{ marginBottom: '0.6rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                                        ...
                                        {cert.date && <span style={{ fontSize: '0.6rem', color: sc, fontFamily: "'DM Mono', monospace" }}>{cert.date}</span>}
                                    </div>
                                    {cert.issuer && <div style={{ fontSize: '0.65rem', color: '#374151' }}>{cert.issuer}</div>}
                                    {cert.link && <div style={{ fontSize: '0.58rem', color: sc, fontFamily: "'DM Mono', monospace" }}>{cert.link}</div>}
                                </div>
                            ))}
                        </div>
                    )}
                                    ...
                                    <div key={lang.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        ...
                                        {lang.proficiency && (
                                            <span style={{
                                                ...
                                                background: acAlpha(sc, 0.1), color: sc,
                            ...
                            {data.awards.map(award => (
                                <div key={award.id} style={{ marginBottom: '0.6rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                                        ...
                                        {award.date && <span style={{ fontSize: '0.6rem', color: sc, fontFamily: "'DM Mono', monospace" }}>{award.date}</span>}
                                    </div>
                    ...
                    {data.experiences.length > 0 && data.experiences.some(e => e.company || e.role) && (
                        <div style={{ marginBottom: '1.25rem' }}>
                            ...
                            {data.experiences.filter(e => e.company || e.role).map(exp => (
                                <div key={exp.id} style={{ marginBottom: '1rem', paddingLeft: '0.75rem', borderLeft: `2px solid ${acAlpha(sc, 0.35)}` }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                                        {exp.role && <div style={{ fontSize: '0.78rem', fontWeight: 700, color: '#0f172a' }}>{exp.role}</div>}
                                        {(exp.start || exp.end) && (
                                            <div style={{ fontSize: '0.62rem', color: sc, fontFamily: "'DM Mono', monospace" }}>
                                    ...
                                    {exp.bullets.filter(Boolean).map((b, i) => (
                                        <div key={i} style={{ display: 'flex', gap: '0.4rem', marginBottom: '0.2rem' }}>
                                            <span style={{ color: sc, fontSize: '0.6rem', marginTop: '0.2rem', flexShrink: 0 }}>▸</span>
                            ...
                            {data.projects.filter(p => p.name || p.description).map(proj => (
                                <div key={proj.id} style={{ marginBottom: '0.85rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                                        ...
                                        {proj.link && <span style={{ fontSize: '0.6rem', color: sc, fontFamily: "'DM Mono', monospace" }}>{proj.link}</span>}
                                    </div>
                                    {proj.tech && (
                                        <div style={{ display: 'flex', gap: '0.25rem', flexWrap: 'wrap', margin: '0.2rem 0' }}>
                                            {proj.tech.split(',').map(t => t.trim()).filter(Boolean).map((t, i) => (
                                                <span key={i} style={{
                                                    fontSize: '0.58rem', padding: '0.1rem 0.4rem',
                                                    background: acAlpha(sc, 0.08), color: sc,
                                                    borderRadius: '3px', fontFamily: "'DM Mono', monospace",
                                                }}>{t}</span>
                                            ))}
                                        </div>
                    ...
                    {data.volunteer && data.volunteer.some(v => v.organization || v.role) && (
                        <div style={{ marginBottom: '1.25rem' }}>
                            ...
                            {data.volunteer.filter(v => v.organization || v.role).map(vol => (
                                <div key={vol.id} style={{ marginBottom: '1rem', paddingLeft: '0.75rem', borderLeft: `2px solid ${acAlpha(sc, 0.2)}` }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                            ...
                            {data.publications.map(pub => (
                                <div key={pub.id} style={{ marginBottom: '0.75rem' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                                        ...
                                        {pub.date && <span style={{ fontSize: '0.6rem', color: sc, fontFamily: "'DM Mono', monospace" }}>{pub.date}</span>}
                                    </div>
                                    {pub.publisher && <div style={{ fontSize: '0.67rem', color: '#374151' }}>{pub.publisher}</div>}
                                    {pub.link && <div style={{ fontSize: '0.6rem', color: sc, fontFamily: "'DM Mono', monospace" }}>{pub.link}</div>}
                                </div>
                            ))}
                        </div>
                    )}
